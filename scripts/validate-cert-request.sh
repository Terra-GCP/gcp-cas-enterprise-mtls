...
# Uses GNU date (-d). On macOS, run via Linux CI or use GNU coreutils (gdate).
set -euo pipefail
...
# Azure Pipelines hosted agents are Linux (GNU date). For macOS locally, use gdate or adjust.
...
IN_WINDOW=false
# If not-after falls in org maintenance window (default: mid-Q4 through mid-Jan), reject.
